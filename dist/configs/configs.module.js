"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ConfigsModule = void 0;
const configs_entity_1 = require("./configs.entity");
const configs_contronller_1 = require("./configs.contronller");
const configs_service_1 = require("./configs.service");
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
let ConfigsModule = class ConfigsModule {
};
ConfigsModule = __decorate([
    common_1.Module({
        imports: [typeorm_1.TypeOrmModule.forFeature([configs_entity_1.Config])],
        controllers: [configs_contronller_1.ConfigController],
        providers: [configs_service_1.ConfigService],
        exports: [typeorm_1.TypeOrmModule]
    })
], ConfigsModule);
exports.ConfigsModule = ConfigsModule;
//# sourceMappingURL=configs.module.js.map